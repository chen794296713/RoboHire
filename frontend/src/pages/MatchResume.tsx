...
import type { MatchResultData } from '../components/MatchResultDisplay';
import JsonViewer from '../components/JsonViewer';
import { generateMatchReport } from '../utils/generateMatchReport';
...
export default function MatchResume() {
  ...
  const handleDownloadReport = () => {
    ...
    const html = generateMatchReport(result.data as MatchResultData);
    const data = result.data as MatchResultData;
    const name = (data.resumeAnalysis?.candidateName || 'candidate').replace(/[^a-zA-Z0-9_-]/g, '_');
    const blob = new Blob([html], { type: 'text/html' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `match-report-${name}.html`;
    a.click();
